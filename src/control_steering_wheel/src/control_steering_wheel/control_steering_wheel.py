...
class ControlSteeringWheel(CompatibleNode):
    """
    Handle the rendering
    """

    def __init__(self, resolution):
        super(ControlSteeringWheel, self).__init__("ControlSteeringWheel")
        self._surface = None
...
class KeyboardControl(object):
    ...
    def set_vehicle_control_manual_override(self, enable):
        ...
        """
        self.hud.notification('Set vehicle control steering wheel override to: {}'.format(enable))
...
def main(args=None):
    ...
    pygame.init()
    pygame.font.init()
    pygame.display.set_caption("CARLA ROS control steering wheel")

    try:
        display = pygame.display.set_mode((resolution['width'], resolution['height']),
                                          pygame.HWSURFACE | pygame.DOUBLEBUF)

        control_steering_wheel = ControlSteeringWheel(resolution)
        clock = pygame.time.Clock()

        executor = roscomp.executors.MultiThreadedExecutor()
        executor.add_node(control_steering_wheel)

        spin_thread = Thread(target=control_steering_wheel.spin)
        spin_thread.start()

        while roscomp.ok():
            clock.tick_busy_loop(60)
            if control_steering_wheel.render(clock, display):
                return
            pygame.display.flip()
